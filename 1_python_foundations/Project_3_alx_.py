...
def _():
    ...
    def tractor_movement_realistic(field, start_direction='right'):
        ...
        for row_idx, row in enumerate(field):
            ...
            should_reverse = (row_idx % 2 != 0) if direction == 'right' else (row_idx % 2 == 0)
            if should_reverse:
                col_indices = reverse_list(col_indices)
...
def _():
    ...
    def field_summary(field):
        if not field:
            return (0, 0, True)
        element_counts = [len(row) for row in field]
        num_rows = len(field)
        total_cells = sum(element_counts)
        is_rectangular = len(set(element_counts)) <= 1
...
def _():
    ...
    def log_trip(tractor_id, **details):
        return {'tractor_id': tractor_id, **details}
    ### END FUNCTION
    return (log_trip,)
